...
function render(data){$('job').classList.add('show');$('jobTitle').textContent=data.title||'Extraction run';$('jobKind').textContent=data.kind||'working';$('jobStatus').textContent=data.current_title?`${data.current_status||'working'} · ${data.current_title}`:'Preparing source list…';$('videos').textContent=data.total??data.videos??'—';$('transcripts').textContent=data.completed??data.transcripts??'—';$('parts').textContent=data.notebook_parts??'—';$('state').textContent=data.status||'running';let pct=data.total?Math.min(100,Math.round(((data.completed||0)/data.total)*100)):4;$('bar').style.width=pct+'%';if(data.error){$('error').textContent=data.error;$('error').classList.add('show')}if(data.status==='complete'){clearInterval(timer);$('bar').style.width='100%';$('links').innerHTML=`<a href="/api/jobs/${data.id}/prompt" target="_blank">Open AI prompt</a><a href="/api/jobs/${data.id}/parts" target="_blank">List NotebookLM parts</a><a href="/api/jobs/${data.id}/summary" target="_blank">Open run summary</a>`}}
